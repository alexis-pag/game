...
class Game:
    ...
    def run(self):
        ...
        pygame.quit()
        # Ajoute ceci dans la classe Game, avant la méthode run()
# --- Admin Panel ---
class AdminPanel:
    def __init__(self, game):
        ...
        self.history = []  # Affiche les commandes précédentes
    ...
    def execute_command(self, cmd):
        ...
        try:
            # --- Joueur ---
            if args[0] == "godmode":
                if args[1] == "on": player.invulnerable = True
                elif args[1] == "off": player.invulnerable = False

            elif args[0] == "set":
                if args[1] == "hp": player.hp = max(0, min(int(args[2]), player.max_hp))
                elif args[1] == "max_hp": 
                    player.max_hp = int(args[2])
                    player.hp = min(player.hp, player.max_hp)
                elif args[1] == "speed": player.speed = float(args[2])
                elif args[1] == "jump": player.jump_power = float(args[2])
                elif args[1] == "dash": player.dash_speed = float(args[2])

            elif args[0] == "tp": 
                player.x = float(args[1])
                player.y = float(args[2])
            
            elif args[0] == "heal": player.hp = player.max_hp
            elif args[0] == "reset_player": 
                self.game.start_game()  # reset player & boss

            elif args[0] == "parry":
                player.parry(args[1] == "on")
            elif args[0] == "charge":
                player.charge_percent = min(float(args[1]), player.max_charge)
            elif args[0] == "basic_attack":
                player.basic_attack()
            elif args[0] == "release_charge":
                attack_data = player.release_charged_attack()
                if attack_data:
                    speed = 15
                    vx = math.cos(attack_data['angle']) * speed
                    vy = math.sin(attack_data['angle']) * speed
                    start_x = player.x + player.width // 2
                    start_y = player.y + player.height // 2
                    self.game.projectiles.append(
                        Projectile(start_x, start_y, vx, vy, attack_data['damage'], 'charged_attack')
                    )

            # --- Boss ---
            elif args[0] == "kill" and args[1] == "boss": boss.hp = 0
            elif args[0] == "boss":
                if args[1] == "hp": boss.hp = min(int(args[2]), boss.max_hp)
                elif args[1] in ["phase1","phase2"]: boss.phase = 1 if args[1]=="phase1" else 2
                elif args[1] == "reset": boss.__init__()
                elif args[1] == "teleport":
                    boss.x = float(args[2])
                    boss.y = float(args[3])
                elif args[1] == "invincible":
                    if args[2] == "on": boss.invulnerable = True
                    elif args[2] == "off": boss.invulnerable = False
                elif args[1] in ["fireball","laser","shockwave"]:
                    self.spawn_boss_projectile(args[1])
                elif args[1] == "summon_minions":
                    for _ in range(int(args[2])):
                        self.game.minions.append(Minion(boss.x, boss.y + 200))
                elif args[1] == "cooldowns" and args[2]=="reset":
                    boss.fireball_cooldown = boss.laser_cooldown = boss.sword_cooldown = 0
                    boss.slam_cooldown = boss.shockwave_cooldown = boss.minion_cooldown = 0
                elif args[1] == "phase":
                    boss.phase = int(args[2])
            ...
            # --- Projectiles / Zones ---
                ...
                y = player.y
                if args[1] == "lava":
                    self.game.fire_zones.append({'rect': pygame.Rect(x-40,y-20,80,40), 'timer':900})
                elif args[1] == "fireball": self.game.projectiles.append(Projectile(x,y,5,0,10,'fireball'))
                elif args[1] == "shockwave": self.game.projectiles.append(Projectile(x,y,8,2,20,'shockwave'))
                elif args[1] == "charged_attack": self.game.projectiles.append(Projectile(x,y,10,0,8,'charged_attack'))
                elif args[1] == "minion": self.game.minions.append(Minion(x,y))
                elif args[1] == "orb": self.game.healing_orbs.append(HealingOrb(x,y))
                elif args[1] == "temp_wall": self.game.temp_walls.append({'rect': pygame.Rect(x,y,20,200), 'timer':480})
            
            elif args[0] == "clear":
                if args[1]=="projectiles": self.game.projectiles.clear()
                elif args[1]=="minions": self.game.minions.clear()
                elif args[1]=="fire": self.game.fire_zones.clear()
                elif args[1]=="orbs": self.game.healing_orbs.clear()

            # --- Game State ---
            elif args[0] == "win": self.game.state = GameState.VICTORY
            elif args[0] == "lose": self.game.state = GameState.GAME_OVER
            elif args[0] == "restart": self.game.start_game()
            elif args[0] == "pause":
                self.game.paused = args[1]=="on"

            # --- Camera & Debug ---
            elif args[0] == "camera":
                if args[1]=="follow":
                    self.game.camera_follow = args[2]=="on"
                else:
                    self.game.camera_x = float(args[1])
                    self.game.camera_y = float(args[2])
            elif args[0] == "debug":
                self.game.debug = args[1]=="on"
            
            else:
                self.history.append("Unknown command: " + cmd)
        
        except Exception as e:
            self.history.append(f"Error: {e}")

    def spawn_boss_projectile(self, type_):
        b = self.game.boss
        x = b.x + b.width // 2
        y = b.y + b.height
        if type_ == "fireball": self.game.projectiles.append(Projectile(x,y,0,5,10,'fireball'))
        elif type_ == "laser": self.game.lasers.append({'rect':pygame.Rect(x,0,50,MAP_HEIGHT),'timer':30,'fire_frame':10})
        elif type_ == "shockwave": self.game.projectiles.append(Projectile(x,y,8,2,20,'shockwave'))

    def draw(self, screen):
        if not self.active:
            return
        panel_rect = pygame.Rect(20, 20, 600, 200)
        pygame.draw.rect(screen, (50,50,50), panel_rect)
        pygame.draw.rect(screen, WHITE, panel_rect, 2)
        
        # Draw input text
        input_surf = self.font.render("> " + self.input_text, True, WHITE)
        screen.blit(input_surf, (30, 180))
        
        # Draw history
        for i, h in enumerate(self.history[-6:]):
            surf = self.font.render(h, True, WHITE)
